...
class FoursquareRepository {
  Future<NearbyPlacesResponse> getNearbyPlaces() async {
    ...
    Map<String, String> header =
        Map<String, String>.from(queryString['nearbyPlace']!['header'] as Map<dynamic, dynamic>);
  ...
  Future<List<PlacesPhotoResponse>> getPlacePhoto(String id) async {
    ...
    Map<String, String> header =
        Map<String, String>.from(queryString['placePhoto']!['header'] as Map<dynamic, dynamic>);
    var photoResponse = await NetworkUtility.fetchUrl(photoUrl, headers: header);
    ...
    List<PlacesPhotoResponse> parsePhotos(String responseBody) {
      ...
      return parsed.map<PlacesPhotoResponse>((json) => PlacesPhotoResponse.fromJson(json)).toList();
    }
  ...
  Future<PlaceDetailsResponse> getPlaceDetails(String id) async {
    ...
    Map<String, String> header =
        Map<String, String>.from(queryString['placeDetails']!['header'] as Map<dynamic, dynamic>);
    var detailsResponse = await NetworkUtility.fetchUrl(url, headers: header);

    return PlaceDetailsResponse.fromJson(jsonDecode(detailsResponse.toString()));
  }
  ...
  Future<SearchPlaceResponse> searchPlace(
    ...
    Map<String, String> header =
        Map<String, String>.from(queryString['searchPlace']!['header'] as Map<dynamic, dynamic>);
  ...
  Future<List<TipResponse>> tip(String placeId) async {
    ...
    Map<String, String> header =
        Map<String, String>.from(queryString['tip']!['header'] as Map<dynamic, dynamic>);
    var response = await NetworkUtility.fetchUrl(url, headers: header);
    List<TipResponse> parseTips(String responseBody) {
      final parsed = json.decode(responseBody).cast<Map<String, dynamic>>();
      return parsed.map<TipResponse>((json) => TipResponse.fromJson(json)).toList();
    }
  ...
  Future<SearchResults?> searchPlaceByLL(String lat, String lng) async {
    var url = Uri.parse('https://api.foursquare.com/v3/places/search?ll=${lat}%2C${lng}');
